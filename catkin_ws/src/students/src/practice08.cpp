...
<<<<<<< HEAD
    for(size_t i=0; i<particles.poses.size();i++){
        simulated_scans[i] = occupancy_grid_utils::simulateRangeScan(map, particles.poses[i],real_sensor_info);//grid, posicion sensor, 
    }
...
<<<<<<< HEAD
    ...
    for(size_t i = 0; i < simulated_scans.size(); i++){
        ...
        for(size_t j = 0; j < simulated_scans[i].ranges.size(); j++{
            ...
            simulated_scans[i].ranges[j] < real_scan.range_max)
                weights[i] += fabs(simulated_scans)
    ...
    for(int i = 0; i < particles.poses.size(); i++){
        for(int j = 0; < simulated_scans[j].ranges.size()){
            simulated = simulated_scans.ranges[j];
...
<<<<<<< HEAD
    ...
    for (size_t i=0; i < particles.poses.size(); i++){
        ...
        resampled_particles.poses[i].position.y = particles.poses[indice].position.y + rnd.gaussian(0,RESAMPLING_NOISE);
...
<<<<<<< HEAD
        ...
        std::normal_distribution<double> distribution(0.0,MOVEMENT_NOISE);
        ...
        a += delta_t; //delta_A en los apuntes

        


        particles.poses[i].position.x += delta_x*cos(a) - delta_y*sin(a) + gaussian_noise; // + (particles.poses[i-1].position.x - particles.poses[i].position.x )
        particles.poses[i].position.y += delta_x*sin(a) + delta_y*cos(a) + gaussian_noise;      
