...
std::vector<float> calculate_particle_weights(std::vector<sensor_msgs::LaserScan>& simulated_scans, sensor_msgs::LaserScan& real_scan)
{
     ...
     */
     ...
     for(int i=0; i<simulated_scans.size(); i++)
     {
         ...
         weights[i] = exp(-(pow(2, diffs))/SENSOR_NOISE);
         sum += weights[i];
         diffs = 0;
     }
...
geometry_msgs::PoseArray resample_particles(geometry_msgs::PoseArray& particles, std::vector<float>& weights)
{
     ...
     */
     ...
     {
          int particle = random_choice(weights);
          
          float a = atan2(particles.poses[particle].orientation.z, particles.poses[particle].orientation.w)*2;
          resampled_particles.poses[i].position.x = particles.poses[particle].position.x + rnd.gaussian(0, RESAMPLING_NOISE);
          resampled_particles.poses[i].position.y = particles.poses[particle].position.y + rnd.gaussian(0, RESAMPLING_NOISE);
