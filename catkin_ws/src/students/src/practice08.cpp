     ...
     */
     ...
     for(int i=0; i < simulated_scans.size(); i++)
     {
        float diffs = 0.0; 
         ...
         {
            float simulated = (simulated_scans[i].ranges[j] < simulated_scans[i].range_max) ? simulated_scans[i].ranges[j] : simulated_scans[i].range_max;
            float real = (real_scan.ranges[j*LASER_DOWNSAMPLING] < real_scan.range_max) ? real_scan.ranges[j * LASER_DOWNSAMPLING] : real_scan.range_max;

            diffs += abs(real - simulated);
         }
        diffs /= simulated_scans[i].ranges.size();
        weights[i] = exp(-(diffs*diffs)/SENSOR_NOISE);
        w_sum += weights[i];
     }
...
geometry_msgs::PoseArray resample_particles(geometry_msgs::PoseArray& particles, std::vector<float>& weights)
{
    ...
    {
        int rand_i = random_choice(weights);
        resampled_particles.poses[i].position.x = particles.poses[rand_i].position.x + rnd.gaussian(0, RESAMPLING_NOISE);
        resampled_particles.poses[i].position.y = particles.poses[rand_i].position.y + rnd.gaussian(0, RESAMPLING_NOISE);

        float a = atan2(particles.poses[rand_i].orientation.z, particles.poses[rand_i].orientation.w) * 2 + rnd.gaussian(0, RESAMPLING_NOISE);
...
int main(int argc, char** argv)
{
     ...
     */
    ros::service::waitForService("/static_map", ros::Duration(20)); //Esperar a que este disponible el mapa
