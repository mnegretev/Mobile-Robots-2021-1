...
def get_cost_map(static_map, r1):
    print "Calculating cost map with " +str(r1) + " cells"
    ...
    for i in range(0,height-1):
        for  j in range(0,width-1):
            if static_map[i,j]>50:
                for k1 in range(-r1,+r1):
                    for k2 in range(-r1,+r1):
                        c= r1+1-max(abs(k1),abs(k2))
