...
def get_inflated_map(static_map, inflation_cells):
    ...
    #
    # Inflated Map
    for i in range(0, height - 1):                                                     # X Axis
        for j in range(0, width - 1):                                                  # Y Axis
            if(static_map[i,j] > 50):
                for k1 in range(i - inflation_cells, i + inflation_cells):             # Inflation Cell (GUI)
                    for k2 in range(j - inflation_cells, j + inflation_cells):         # Inflation Cell (GUI)
                        inflated[k1,k2] = 100                                          # Map Inflated         
            ...
            if(static_map[i,j] > 50):
                for k1 in range(-cost_radius, cost_radius):
                    for k2 in range(-cost_radius, cost_radius):
                        ...
                        cost_map [i + k1, j + k2] = max(c, cost_map[i + k1, j + k2])   # Map Cost
