...
def get_inflated_map(static_map, inflation_cells):
    ...
    for x in range(0, height - 1):
        for y in range(0, width - 1):
            if(static_map[x, y] > 50):
                for inflatedX in range(x -inflation_cells, x + inflation_cells):
                    for inflatedY in range(y - inflation_cells, y + inflation_cells):
                        inflated[inflatedX, inflatedY] = 100
...
def get_cost_map(static_map, cost_radius):
    ...
    for x in range(0, height - 1):
        for y in range(0, width - 1):
            if(static_map[x, y] > 50):
                for costX in range(-cost_radius, cost_radius):
                    for costY in range(-cost_radius, cost_radius):
                        cost = cost_radius + 1 - max(abs(costX), abs(costY))
                        cost_map[x + costX, y + costY] = max(cost, cost_map[x + costX, y + costY])
