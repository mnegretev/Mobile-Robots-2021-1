...
def evaluate(weights,image):
    ...
    th = 0.6
    u = numpy.dot(image,weights)-th
    out_p = 1/(1+numpy.exp(-u))
	...
	gradient = numpy.zeros(len(weights))	
        for i in range(len(inputs)):
	      y_hat = evaluate(weights,inputs[i])
              ...
              g_j = (y_hat - y)*(y_hat*(1-y_hat))*inputs[i]
              gradient = numpy.add(gradient,g_j)
	      
      
        weights = numpy.add(weights,-epsilon*gradient)
        gradient_mag = numpy.linalg.norm(gradient,2)
        attempts = attempts - 1

        print('|gradient| :',gradient_mag,' attempts :',attempts)    
