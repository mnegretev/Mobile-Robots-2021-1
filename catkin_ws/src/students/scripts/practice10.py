...
NAME = "FERNANDEZ_MORA_JOSE_ENRIQUE"
...
def train_perceptron(weights, images, labels, desired_digit):
    ...
    #     
    mag=1
    while mag>tol and attempts>0 and not rospy.is_shutdown():
        ...
        for x in inputs:
            ...
            y=0
            #labels(int(x/7480))???
            if(labels[i]==desired_digit):
                y=1
            gradient=numpy.add(gradient,(y_hat-y)*y_hat*(1-y_hat)*x)
            i+=1
	weights=numpy.add(weights,-epsilon*gradient)
        attempts=attempts-1                    
        mag=numpy.linalg.norm(gradient)
        print('gradient magnitud: '+str(mag)+'attemps: '+str(attempts))
        os.system('clear')
