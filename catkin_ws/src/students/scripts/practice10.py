...
def train_perceptron(weights, images, labels, desired_digit):
    ...
    mag_grad = 1
    y = 0
    while  mag_grad > tol and attempts > 0 and not rospy.is_shutdown():
	gradient = numpy.zeros(len(weights))
        i = 0
	for img in inputs:
	    y_hat = evaluate(weights, img)
            if labels[i] == desired_digit:
		y = 1 
	    else: 
		y = 0
	    g_j = ((y_hat - y) * (y_hat * (1 - y_hat))) * img  
	    gradient = numpy.add(gradient, g_j)
	    i+=1
	weights = numpy.add(weights, -(epsilon*gradient))
	attempts = attempts - 1
        mag_grad = numpy.linalg.norm(gradient)
    return weights
