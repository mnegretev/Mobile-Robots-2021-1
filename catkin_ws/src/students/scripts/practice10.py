...
def evaluate(weights, image):
    ...
    #    
    dotProduct = numpy.dot(weights, image)
    function = (1 / (1 + math.exp(-dotProduct)))
    return function
    #return 0.5
...
def train_perceptron(weights, images, labels, desired_digit):
    ...
    #     attempts = attempts - 1
    norm = 1        #Initial value to enter the loop
    while(norm > tol and attempts > 0 and not rospy.is_shutdown()):
        gradient = numpy.zeros(len(weights))    #gradient = [0, 0, ... 0]
        i = 0           #Initial value to check the label 
        for x in inputs:
            y_hat = evaluate(weights, x)
            y = 1 if (labels[i] == desired_digit) else 0
            constant = (y_hat - y)*y_hat*(1 - y_hat)
            g_j = constant*x
            gradient = numpy.add(gradient, g_j)
            i += 1
        weights = numpy.add(weights, -epsilon*gradient)
        norma = numpy.linalg.norm(gradient)

    	attempts -= 1
        #For debug
        print 'Gradient magnitude: ' + str(norma) + ' Attemps left: ' + str(attempts)
    return weights
