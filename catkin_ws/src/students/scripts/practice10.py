...
def evaluate(weights, image):
    ...
    #
    u = numpy.dot(weights,image)
    #print(u)
    output = 1 / (1 + (math.exp(-u)))
    return output
...
def train_perceptron(weights, images, labels, desired_digit):
    ...
    while numpy.linalg.norm(gradiente) > tol and attempts >0 and not rospy.is_shutdown():
        gradiente = numpy.zeros(len(weights))
        aux=0
        for x in inputs:
            y_hat = evaluate(weights,x)
            y = 1 if labels[aux] == desired_digit else 0
            #if labels[numpy.assarray(inputs.index(x))] == desired_digit:
            #    y = 1
            #else:
                #y= 0
            g_j = x*(y_hat - y)*(y_hat*(1-y_hat))
            gradiente = numpy.add(gradiente,g_j)
            aux=aux+1
        weights = numpy.add(weights, -epsilon*gradiente)
        attempts = attempts -1
