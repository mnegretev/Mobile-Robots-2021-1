...
def evaluate(weights, image):
    ...
    #    
    r = numpy.dot(weights, image)
    output = 1 / (1 + numpy.exp(-r))
    return output
...
def train_perceptron(weights, images, labels, desired_digit):
    ...
    grd = 1
    while grd > tol and attempts > 0 and not rospy.is_shutdown():
        grd_v = numpy.zeros(len(weights))
        i = 0
        for x in inputs:
            y_hat = evaluate(weights, x)
            y = 1 if labels[i] == desired_digit else 0
            g_j = (y_hat - y) * y_hat * (1 - y_hat) * x
            grd_v = grd_v + g_j
            i += 1
        weights = weights - epsilon * grd_v
        attempts = attempts - 1
        grd = numpy.linalg.norm(grd_v)
        print ("Valor del gradiente: " + str(grd) + " Intentos restantes: " + str(attempts))
