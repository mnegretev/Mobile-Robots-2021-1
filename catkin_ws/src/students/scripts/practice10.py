...
def train_perceptron(weights, images, labels, desired_digit):
    ...
    while numpy.linalg.norm(gradient) > tol and attempts > 0 and not rospy.is_shutdown():     
        ...
        for i in range(len(inputs)):
            y_hat = evaluate(weights,inputs[i])
            y     = 1 if labels[i] == desired_digit else 0
            g_j   = inputs[i]*((y_hat - y)*(y_hat*(1-y_hat)))
            gradient = numpy.add(gradient, g_j)
        weights = numpy.add(weights, gradient*(-epsilon))
        attempts = attempts - 1
        print('Gradient magnitude: {:12.10f}  Remaining attempts: {}'.format(numpy.linalg.norm(gradient),attempts))
    return weights
