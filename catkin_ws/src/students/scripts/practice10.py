...
def evaluate(weights, image):
    ...
    #
    return (1 / (1 + math.exp(-numpy.dot(weights, image))))
...
def train_perceptron(weights, images, labels, desired_digit):
    ...
    gradient = numpy.zeros(len(weights)-1) + [1]

    #
    # WHILE |gradient| > tol and attempts > 0 and not rospy.is_shutdown():
    while numpy.linalg.norm(gradient) > tol and attempts > 0 and (not rospy.is_shutdown()):
        #Set gradient to vector zero
        gradient = numpy.zeros(len(weights))
        #FOR EACH x IN inputs:
        for x in range(len(inputs)):
            #y_hat = Perceptron's output for input 'x' with the current weights
            y_hat = evaluate(weights, inputs[x])
            #y     = Desired output (1 if label for img corresponds to the desired digit. 0, otherwise)
            y = 1 if labels[x] == desired_digit else 0
            #g_j   = Gradient term corresponding to input 'img'
            g_j  = ((y_hat - y)*(y_hat-y_hat*y_hat)) * inputs[x]
        ...
        print("Gradient magnitude: " + str(numpy.linalg.norm(gradient)) + "Remaining attempts: " + str(attempts))
        #attempts = attempts - 1
        attempts = attempts - 1
    #
    return weights
