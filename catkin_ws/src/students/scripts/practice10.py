...
def evaluate(weights, image):
    ...
    u = numpy.dot(weights, image)										# Dot Product
    sigmoid = (1 / (1 + math.exp(-u)))									# Sigmoid Function
    return sigmoid
...
def train_perceptron(weights, images, labels, desired_digit):
    ...
    grad_magnitude = 1																					# Initial value for grad_magnitude

    while grad_magnitude > tol and attempts > 0 and not rospy.is_shutdown():
    	gradient = numpy.zeros(len(weights))															# gradient = [0,0,0,0,0,0,0,.......]
    	index = 0																						# index to labels
    	for x in inputs:	
    		y_hat = evaluate(weights, x)																# Perceptron's output
    		y = 1 if (labels[index] == desired_digit) else 0											# Desired Output
    		g_j = ((y_hat - y) * y_hat * (1 - y_hat)) * x												# Constant g_j * x
    		gradient = numpy.add(gradient, g_j)															# gradient = gradient + g_j
    		index += 1

    	weights = numpy.add(weights, -epsilon*gradient)													# weights = weights - epsilon*gradient
    	grad_magnitude = numpy.linalg.norm(gradient)
    	attempts -= 1
    	print("Gradient Magnitude: " + str(grad_magnitude) + " " +" # Attempts: " + str(attempts))		# Show Gradient Magnitude and # Attempts 
