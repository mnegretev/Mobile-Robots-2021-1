...
def evaluate(weights, image):
    ...
    #salida tiene que ser sigmoide
    p_punto =  numpy.dot(weights, image) #producto punto
    p_otput = 1 / (1 + m.exp(-1 * p_punto))  
    return p_otput#si es mayor a 0.5 la imagen corresponde a digito entrenado

#labels son diditos de entre 0 y 9
def train_perceptron(weights, images, labels, desired_digit):
    ...
    #         y     = 1 if(label = ) else 0 ----- Desired output (1 if label for img corresponds to the desired digit. 0, otherwise)
    #         g_j   = Gradient term corresponding to input 'img'
    #         gradient = gradient + g_j
    #     weights = weights - epsilon*gradient # usar numpy.add
    #     attempts = attempts - 1
    #     
    m_grad = 1
    ...
    while m_grad > tol and attempts > 0 and not rospy.is_shutdown():
        ...
        i = 0
        for x in inputs:
            y_hat = evaluate(weights,x) #Perceptron's output for input signal x with the current weights
            y     = 1 if(labels[i] == desired_digit) else 0 # Desired output (1 if label for img corresponds to the desired digit. 0, otherwise)
            g_j   = x * (y_hat - y) * (y_hat * (1 - y_hat))
            gradient = gradient + g_j #np.add(gradient, g_j)
            i += 1
        m_grad = numpy.linalg.norm(gradient) #magnitud del gradiente
        weights = numpy.add(weights, -epsilon * gradient) # usar numpy.add
        attempts = attempts - 1
        print(attempts)
