...
def calculate_control(robot_x, robot_y, robot_a, goal_x, goal_y):
    ...
    cmd_vel.angular.x = min(v_max, error_a)*math.exp(-error_a*error_a/alpha)
...
def attraction_force(robot_x, robot_y, goal_x, goal_y):
    ...
    tau = 1.0
    magnitud = (math.sqrt( pow((goal_x - robot_x),2) + pow(goal_y - robot_y,2) ))
    [force_x, force_y] = [tau*(goal_x - robot_x) / magnitud , tau*(goal_y - robot_y) / magnitud]
...
def rejection_force(robot_x, robot_y, robot_a, laser_readings):
    ...
    for i in range(len(laser_readings)):
        ...
        if distance < d0:
            Frx = Frx + beta*math.sqrt(1/distance -1/d0) * math.cos(angle + robot_a)
            Fry = Fry + beta*math.sqrt(1/distance -1/d0) * math.sin(angle + robot_a)
        if distance >= d0:
            Frx = Frx + 0;
            Fry = Fry + 0;

    force_x = Frx/len(laser_readings)
    force_y = Fry/len(laser_readings)
...
def callback_pot_fields_goal(msg):
    ...
    epsilon=0.5 # epsilon de la practica 4
    tolerance = 0.1
