...
def rejection_force(robot_x, robot_y, robot_a, laser_readings):
    ...
    for i in range(len(laser_readings)-1):
        dis,ang=laser_readings[i]

        if dis<1:
            Fx=Fx+beta*math.sqrt(1/dis -1/1)*math.cos(ang+robot_a)
            Fy=Fy+beta*math.sqrt(1/dis -1/1)*math.sin(ang+robot_a)
        if dis>=1:
            Fx=Fx+0;
            Fy=Fy+0;
...
def callback_pot_fields_goal(msg):
    ...
    while goal_distance > 0.1 and not rospy.is_shutdown():
    	...
    	pub_cmd_vel.publish(calculate_control(robot_x, robot_y, robot_a, px, py))					
