...
def calculate_control(robot_x, robot_y, robot_a, goal_x, goal_y):
    ...
    alpha = 0.1
    beta = 1
    #Constantes segun las caracteristicas del motor
    v_max = 0.7
    w_max = 0.5
    #Calculando el error de angulo
    error_a = math.atan2(goal_y-robot_y,goal_x-robot_x) - robot_a
    #Para mantener el error entre -Pi y Pi
    if(error_a > math.pi): error_a = error_a - 2 * math.pi
    if(error_a < - math.pi): error_a = error_a + 2 * math.pi
    #Leyes de control
    v = v_max*math.exp(-error_a*error_a/alpha)
    w = w_max*(2/(1 + math.exp(-error_a/beta)) - 1)
    #Asignar el valor de velocidad lineal y angular
    cmd_vel.linear.x = v
    cmd_vel.angular.z = w
...
def attraction_force(robot_x, robot_y, goal_x, goal_y):
    ...
    alpha = 1.5 #Constante
    K = alpha / (math.sqrt(((robot_x - goal_x)**2)+((robot_y - goal_y)**2))) #Magnitud de la fuerza
...
def rejection_force(robot_x, robot_y, robot_a, laser_readings):
    ...
    #
    beta = 1.5 #Constante
    d0 = 1.1 #Distancia de influencia
    #Inicializamos fureza repulsiva
    force_x = 0
    force_y = 0
    for reading in laser_readings: #Para cada lectura del laser
        [d, angle] = reading #Tomamos un valor de la lectura
        if d <= d0:
            Fr_mag = beta * math.sqrt(1/d - 1/d0) #Obtenemos la magnitud de la fuerza repulsiva
            #Sumamos las fuerzas repulsivas
            force_x +=Fr_mag * math.cos(angle + robot_a)
            force_y +=Fr_mag * math.sin(angle + robot_a)
    #Calculamos el promedio de las fuerzas repulsivas
...
def callback_pot_fields_goal(msg):
    ...
    tol = 0.1 #Tolerancia
    robot_x, robot_y, robot_a = get_robot_pose(listener) #Obtener la posicion del robot
    distance_to_goal_point = math.sqrt((goal_x - robot_x)**2 + (goal_y - robot_y)**2) #Distancia al punto meta
    while distance_to_goal_point > tol and not rospy.is_shutdown():
        [fax, fay] = attraction_force(robot_x, robot_y, goal_x, goal_y) #Calcular fuerza atractiva
        [frx, fry] = rejection_force (robot_x, robot_y, robot_a, laser_readings) #Calcular la fuerza repulsiva
        [fx, fy] = [fax + frx, fay + fry] #Calculamos la fuerza resultante
        [px,py] = [robot_x - epsilon * fx, robot_y - epsilon * fy] #Calculamos el siguiente punto meta

        msg_cmd_vel = calculate_control(robot_x, robot_y, robot_a, px, py) #Calculamos la senial de control
        pub_cmd_vel.publish(msg_cmd_vel) #Publicamos la senial de control para mover el robot
        draw_force_markers(robot_x, robot_y, fax, fay, frx, fry, fx, fy, pub_markers) #Mostrar las fuerzas que se estan calculando

        loop.sleep() #Pausa
        robot_x, robot_y, robot_a = get_robot_pose(listener) #Actualizar posicion del robot
        distance_to_goal_point = math.sqrt((goal_x - robot_x)**2 + (goal_y - robot_y)**2) #Recalculamos distancia al punto meta
    
    pub_cmd_vel.publish(Twist()) #Detenemos el robot
