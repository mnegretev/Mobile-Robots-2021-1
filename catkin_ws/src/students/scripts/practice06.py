...
def attraction_force(robot_x, robot_y, goal_x, goal_y):
    ...
    modulo = math.sqrt((force_x * force_x) + (force_y * force_y))
    if modulo == 0:
        return [0, 0]
    [force_x, force_y] = [alpha * (force_x / modulo), alpha * (force_y / modulo)]
...
def rejection_force(robot_x, robot_y, robot_a, laser_readings):
    ...
    for [d, angulo] in laser_readings:
        if d > 0 and d < d0:
            modulo = beta * math.sqrt((1 / d) - (1 / d0))
        else:
            modulo = 0
        force_x = force_x + modulo*math.cos(angulo + robot_a)
        force_y = force_y + modulo*math.sin(angulo + robot_a)
