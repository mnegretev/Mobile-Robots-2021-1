...
def calculate_control(robot_x, robot_y, robot_a, goal_x, goal_y):
    ...
    error_a = math.atan2(error_y, error_x) - robot_a        # Calculate Angle Error
    if(error_a < -math.pi):                                 # Error Angle (-pi,pi]
        error_a = error_a + (2*math.pi)
    if(error_a > math.pi):
        error_a = error_a - (2*math.pi)

    v = v_max*math.exp(-error_a*error_a/alpha)              # Control Law v                  
    w = w_max*(2/(1 + math.exp(-error_a/beta)) - 1)         # Control Law w

    cmd_vel.linear.x = v                                    # Twist Message cmd_vel v
    cmd_vel.angular.z = w                                   # Twist Message cmd_vel w
...
def attraction_force(robot_x, robot_y, goal_x, goal_y):
    ...
    #
    alpha = 1.0																	# Alpha
    qNorm = math.sqrt((robot_x - goal_x)**2 + (robot_y - goal_y)**2)			# Unitary Vector
    force_x = alpha*((robot_x - goal_x) / qNorm)								# Atraction Force X Component
    force_y = alpha*((robot_y - goal_y) / qNorm)								# Atraction Force Y Component

    return [force_x, force_y]													# Atraction Force
...
def rejection_force(robot_x, robot_y, robot_a, laser_readings):
    ...
    d0 = 1																	# d0 1[m]
    beta = 5
    force_x = 0																# Rejection Force X Component
    force_y = 0																# Rejection Force Y Component
    for [d,a] in laser_readings:											# [d, a] Distance and Angle
    	if d < d0 and d > 0:
    		mag = beta * math.sqrt((1/d) - (1/d0))							# Calculation of mag 
    	else:
    		mag = 0															# Set mag = 0

    	force_x += math.cos(robot_a + a) * mag 								# Calculation of Force X Component 
    	force_y += math.sin(robot_a + a) * mag 								# Calculation of Force Y Component

    force_x = force_x / len(laser_readings)									# AVG Force X Component
    force_y = force_y / len(laser_readings)									# AVG Force Y Comopnent
...
def callback_pot_fields_goal(msg):
    ...
    goal_distance = math.sqrt((goal_x - robot_x)**2 + (goal_y - robot_y)**2)						# Distance to Goal

    while goal_distance > tolerance and not rospy.is_shutdown():
    	[fax, fay] = attraction_force(robot_x, robot_y, goal_x, goal_y)								# Calculation of Attraction Force
    	[frx, fry] = rejection_force(robot_x, robot_y, robot_a, laser_readings)						# Calculation of Rejection Force
    	F = [fax + frx, fay + fry]																	# Resultant Force 
    	[px, py] = [robot_x - (epsilon*(F[0])), robot_y - (epsilon*(F[1]))]							# Next Local Goal Point
    	pub_cmd_vel.publish(calculate_control(robot_x, robot_y, robot_a, px, py))					# Calculaion of Control Signals and Publish Message
    	draw_force_markers(robot_x, robot_y, fax, fay, frx, fry, F[0], F[1], pub_markers)			# Draw Forces
    	loop.sleep()																				# Wait
    	[robot_x, robot_y, robot_a] = get_robot_pose(listener)										# Update Robot Position
    	goal_distance = math.sqrt((goal_x - robot_x)**2 + (goal_y - robot_y)**2)					# Update Distance to Goal
