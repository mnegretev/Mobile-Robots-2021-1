...
def rejection_force(robot_x, robot_y, robot_a, laser_readings):
    ...
    for i in range(len(laser_readings)):
        ...
        if di > d0:
            fxi,fyi = 0,0
        if di < d0 and di>0:
            fxi,fyi = beta*math.sqrt(1/di - 1/d0)*(math.cos(anglei+robot_a)),beta*math.sqrt(1/di - 1/d0)*(math.sin(anglei+robot_a))
        auxx,auxy = auxx+fxi,auxy+fyi
    force_x,force_y = auxx/len(laser_readings),auxy/len(laser_readings)
    return [force_x, force_y]
...
def callback_pot_fields_goal(msg):
    ...
    d2g = math.sqrt((goal_x - robot_x)**2 + (goal_y - robot_y)**2)
    while d2g > tolerance and not rospy.is_shutdown():
        [fax, fay] = attraction_force(robot_x, robot_y, goal_x, goal_y)
        [frx, fry] = rejection_force (robot_x, robot_y, robot_a, laser_readings)
        F = [fax+frx, fay+fry]
        Pl = Pr[0]-epsilon*F[0],Pr[1]-epsilon*F[1]
        #Pl = Pr - epsilon*F
        px,py = Pl
