...
def attraction_force(robot_x, robot_y, goal_x, goal_y):
    ...
    force_x= alpha*((robot_x - goal_x)/((robot_x - goal_x)**2+(robot_y - goal_y)**2)**0.5)
    force_y= alpha*((robot_y - goal_y)/((robot_x - goal_x)**2+(robot_y - goal_y)**2)**0.5)
...
def rejection_force(robot_x, robot_y, robot_a, laser_readings):
    ...
    for read in range(len(laser_readings)):
        distance,angle=laser_readings[read]

        if distance<1:
            Fx = Fx+beta*math.sqrt((1/distance)-(1/influence))*math.cos(angle+robot_a)
            Fy = Fy+beta*math.sqrt((1/distance)-(1/influence))*math.sin(angle+robot_a)
        if distance>=1:
            Fx = Fx+0;
            Fy = Fy+0;
