...
def follow_path(path):
    ...
    E = 0.3 #Epsilon
    i = 0 #Primer indice
    local_goal = path[i] #Tomamos el primer punto de la trayectoria
    global_goal = path[-1] #Tomamos el ultimo punto de la trayectoria
    Pr = get_robot_pose(listener) #Obtenemos la posicion del robot
    e_global = math.sqrt(((Pr[0]-global_goal[0])**2)+((Pr[1]-global_goal[1])**2)) #Error global
    e_local = math.sqrt(((Pr[0]-local_goal[0])**2)+((Pr[1]-local_goal[1])**2)) #Error local

    while e_global > tol and i < (len(path)-1) and not rospy.is_shutdown():
        pub_cmd_vel.publish(calculate_control(Pr[0],Pr[1],Pr[2],local_goal[0],local_goal[1])) #Calculamos las leyes de control y publicamos el resultado para que el robot se mueva
        loop.sleep()
        Pr = get_robot_pose(listener) #Obtenemos la posicion del robot
        e_local = math.sqrt(((Pr[0]-local_goal[0])**2)+((Pr[1]-local_goal[1])**2)) #Error local
        if e_local < E:
            i = i+1
            local_goal = path[i] #Cambiamos al siguiente punto
        e_global = math.sqrt(((Pr[0]-global_goal[0])**2)+((Pr[1]-global_goal[1])**2)) #Error global
