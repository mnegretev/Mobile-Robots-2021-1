...
def calculate_control(robot_x, robot_y, robot_a, goal_x, goal_y):
    ...
    w = w_max*(2/(1 + math.exp(-error_a/beta)) - 1)         # Control Law
...
def follow_path(path):
    ...
    #
    i = 0
    tolerance = 0.2                                                                                         # Tolerance Value
    last_global_goal = len(path) - 1                                                                        # Last Global Goal Point

    [local_goal_x, local_goal_y] = path[0]                                                                  # Local Goal First Point
    [global_goal_x, global_goal_y] = path[last_global_goal]                                                 # Global Goal Last Point
    [robot_x, robot_y, robot_a] = get_robot_pose(listener)                                                  # Get Robot Position
    global_error = ((robot_x - global_goal_x)**2 + (robot_y - global_goal_y)**2)**0.5                       # Calculating Global Error
    local_error = ((robot_x - local_goal_x)**2 + (robot_y - local_goal_y)**2)**0.5                          # Calculating Local Error

    while global_error > tolerance and not rospy.is_shutdown():
        pub_cmd_vel.publish(calculate_control(robot_x, robot_y, robot_a, local_goal_x, local_goal_y))       # Calculating Control Signals and Publishing Message
        loop.sleep()
        [robot_x, robot_y, robot_a] = get_robot_pose(listener)                                              # Get Robot Position
        if(local_error < tolerance):
            i = i + 1
            [local_goal_x, local_goal_y] = path[i]                                                          # Local Goal Point <- Path + 1 (Next Point)        
        global_error = math.sqrt(((robot_x - global_goal_x)**2 + (robot_y - global_goal_y)**2))             # Calculating Global Error (Update)
        local_error = math.sqrt(((robot_x - local_goal_x)**2 + (robot_y - local_goal_y)**2))                # Calculating Local Error (Update)
