...
def dijkstra(start_r, start_c, goal_r, goal_c, grid_map, cost_map):
    ...
    execution_steps=0
    open_list      = []
    heapq.heapify(open_list)                            ##### Priority Queue
    ...
    while len(open_list) > 0 and [r,c] != [goal_r, goal_c]:
        (weight, [r,c]) = heapq.heappop(open_list)      ##### Pop Priority Queue (weight, [r,c])
        ...
        for [nr,nc] in neighbors:
            if grid_map[nr,nc] > 40 or grid_map[nr,nc] < 0 or in_closed_list[nr,nc]:
                continue
            g = distances[r,c] + 1 + cost_map[nr,nc]    ##### Calculate g Value
            if g < distances[nr,nc]:
                distances[nr,nc]    = g
                parent_nodes[nr,nc] = [r,c]
            if not in_open_list[nr,nc]:
                in_open_list[nr,nc] = True
                heapq.heappush(open_list,(g,[nr,nc]))   ##### Push Priority Queue (weight, [nr,nc])
            execution_steps += 1
...
def a_star(start_r, start_c, goal_r, goal_c, grid_map, cost_map):
    ...
    while len(open_list) > 0 and [r,c] != [goal_r, goal_c]:
        (weight, [r,c]) = heapq.heappop(open_list)          ##### Pop Priority Queue
        in_closed_list[r,c] = True
        neighbors = [[r+1, c],  [r,c+1],  [r-1, c],  [r,c-1]]
        ...
        for [nr,nc] in neighbors:
            if grid_map[nr,nc] > 40 or grid_map[nr,nc] < 0 or in_closed_list[nr,nc]:
                continue
            g = distances[r,c] + 1 + cost_map[nr,nc]        ##### Calculate g Value
            h = abs(r - goal_r) + abs(c - goal_c)           ##### Manhatan Distance
            f = g + h                                       ##### Calculate f Value
            if g < distances[nr,nc]:
                distances[nr,nc]    = g
                parent_nodes[nr,nc] = [r,c]
            if not in_open_list[nr,nc]:
                in_open_list[nr,nc] = True
                heapq.heappush(open_list,(f,[nr,nc]))       ##### Push Priority Queue (weight, [nr,nc])
            execution_steps += 1
...
def get_smooth_path(original_path, alpha, beta):
    ...
    while gradient_mag > tolerance:
        for i in range(1, len(original_path) - 1):          # Calculate all components except first one and last one
            [xo_i, yo_i] = original_path[i]                 # Original Path (i- th Point)
            [xn_i, yn_i] = smooth_path[i]                   # Smooth Path (i - th Point) New
            [xn_in, yn_in] = smooth_path[i+1]               # Smooth Path (i + 1  - th Point) Next
            [xn_io, yn_io] = smooth_path[i-1]               # Smooth Path (i - 1  - th Point) Old

            grad_x = alpha*(xn_i - xo_i) + beta*(2*xn_i - xn_io - xn_in)    # Calculate Gradiant of X component
            grad_y = alpha*(yn_i - yo_i) + beta*(2*yn_i - yn_io - yn_in)    # Calculate Gradient of Y component
            gradient[i] = [grad_x, grad_y]

        smooth_path[0][0] = smooth_path[0][0] - epsilon*(alpha*(smooth_path[0][0] - original_path[0][0]) - beta*(smooth_path[1][0] - smooth_path[0][0]))        # First Point of X
        smooth_path[0][1] = smooth_path[0][1] - epsilon*(alpha*(smooth_path[0][1] - original_path[0][1]) - beta*(smooth_path[1][1] - smooth_path[0][1]))        # First Point of Y

        for i in range(1,len(smooth_path) - 1):
            smooth_path[i][0] -= epsilon*gradient[i][0]                     # For Intermediate  X Points 
            smooth_path[i][1] -= epsilon*gradient[i][1]                     # For Intermediate  Y Points

        smooth_path[last][0] = smooth_path[last][0] - epsilon*(alpha*(smooth_path[last][0] - original_path[last][0]) + beta*(smooth_path[last][0] - smooth_path[last - 1][0]))      # Last Point of X
        smooth_path[last][1] = smooth_path[last][1] - epsilon*(alpha*(smooth_path[last][1] - original_path[last][1]) + beta*(smooth_path[last][1] - smooth_path[last - 1][1]))      # Last Point of Y

        
        gradient_mag = 0                                                    # Calculating New Gradient
        for i in range(len(gradient)):
            gradient_mag += abs(gradient[i][0]) + abs(gradient[i][1])       # Using Norm 1 |grad_x| + |grad_y|
