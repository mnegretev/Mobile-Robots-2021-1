...
def dijkstra(start_r, start_c, goal_r, goal_c, grid_map, cost_map):
    ...
    #
    execution_steps=0
    open_list      = [] # Se crea una lista vacia
    heapq.heapify(open_list) # Transforma la lista abierta en un heap
    ...
    [r,c] = [start_r, start_c]
    heapq.heappush(open_list, (0, [start_r, start_c])) # Se agrega el inicio con costo 0
    in_open_list[start_r, start_c] = True
    distances   [start_r, start_c] = 0

    while len(open_list) > 0 and [r,c] != [goal_r, goal_c]:
        [r,c] = heapq.heappop(open_list)[1] # Obtener el menor valor de la lista heap
        in_closed_list[r,c] = True
        neighbors = [[r+1, c],  [r,c+1],  [r-1, c],  [r,c-1]]
        for [nr,nc] in neighbors:
            if grid_map[nr,nc] > 40 or grid_map[nr,nc] < 0 or in_closed_list[nr,nc]:
                continue
            g = distances[r,c] + 1 + cost_map[nr][nc] # Valor g se calcula como el anterior + 1 + costo
            if g < distances[nr,nc]:
                distances[nr,nc]    = g
                parent_nodes[nr,nc] = [r,c]
            if not in_open_list[nr,nc]:
                in_open_list[nr,nc] = True
                heapq.heappush(open_list, (g, [nr, nc])) # Agregamos si no esta en la lista abierta indicandole
            execution_steps += 1                   # indicandole su costo para que se respete el orden del heap
...
def a_star(start_r, start_c, goal_r, goal_c, grid_map, cost_map):
    ...
    while len(open_list) > 0 and [r,c] != [goal_r, goal_c]:
        ...
        for [nr,nc] in neighbors:
            if grid_map[nr,nc] > 40 or grid_map[nr,nc] < 0 or in_closed_list[nr,nc]:
                continue
            g = distances[r,c] + 1 + cost_map[nr][nc] # Valor g se calcula como el valor anterior + 1 + costo
            h = abs(goal_r - nr) + abs(goal_c - nc) # Heuristica distancia de Manhattan
            f = g + h # Nuevo criterio para agregar elementos al heap
            if g < distances[nr,nc]:
                distances[nr,nc]    = g
                parent_nodes[nr,nc] = [r,c]
            if not in_open_list[nr,nc]:
                in_open_list[nr,nc] = True
                heapq.heappush(open_list, (f, [nr, nc])) # Agregamos si no esta en la lista abierta indicandole
            execution_steps += 1                   # indicandole su costo para que se respete el orden del heap
...
def get_smooth_path(original_path, alpha, beta):
    ...
    while gradient_mag > tolerance:
        ...
        for i in range(1, len(original_path)-1):
            ...
            [xn_sig, yn_sig] = smooth_path[i+1] # para obtener el siguiente punto
