...
def dijkstra(start_r, start_c, goal_r, goal_c, grid_map, cost_map):
    ...
    [r,c] = [start_r, start_c]
    heapq.heappush(open_list, [0, start_r, start_c])   ##### Indice 0 es el valor g
    in_open_list[start_r, start_c] = True
    distances   [start_r, start_c] = 0

    while len(open_list) > 0 and [r,c] != [goal_r, goal_c]:
        r_temp = open_list[0][1]
        c_temp = open_list[0][2]
        ...
        for [nr,nc] in neighbors:
            if grid_map[nr,nc] > 40 or grid_map[nr,nc] < 0 or in_closed_list[nr,nc]:
                continue
            g = distances[r,c] + 1 + cost_map[nr,nc]   ##### Calcular g
            if g < distances[nr,nc]:
                distances[nr,nc]    = g
                parent_nodes[nr,nc] = [r,c]
            if not in_open_list[nr,nc]:
                in_open_list[nr,nc] = True
                heapq.heappush(open_list, [g, nr, nc])     #### Anadir a la open_list con valor g
            execution_steps += 1
...
def get_smooth_path(original_path, alpha, beta):
    ...
    while gradient_mag>tolerance:
        ...
        smooth_path[l][0]=smooth_path[l][0]-epsilon*(alpha*(smooth_path[l][0]-original_path[l][0])+beta*(smooth_path[l][0]-smooth_path[l-1][0]))
        smooth_path[l][1]=smooth_path[l][1]-epsilon*(alpha*(smooth_path[l][1]-original_path[l][1])+beta*(smooth_path[l][1]-smooth_path[l-1][1]))
