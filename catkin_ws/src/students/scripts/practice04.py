...
def dijkstra(start_r, start_c, goal_r, goal_c, grid_map, cost_map):
    ...
    [r,c] = [start_r, start_c]
    heapq.heappush(open_list,(0,[start_r, start_c])) # Se agrega el nodo inicial al heap
    in_open_list[start_r, start_c] = True
    distances   [start_r, start_c] = 0

    while len(open_list) > 0 and [r,c] != [goal_r, goal_c]:
        dist,[r,c] = heapq.heappop(open_list)  ######## HINT
        ...
        for [nr,nc] in neighbors:
            if grid_map[nr,nc] > 40 or grid_map[nr,nc] < 0 or in_closed_list[nr,nc]:
                continue
            g = distances[r,c] + 1 + cost_map[nr,nc] # Se calcula el valor g (se consideran los pesos)
            if g < distances[nr,nc]:
                distances[nr,nc]    = g
                parent_nodes[nr,nc] = [r,c]
            if not in_open_list[nr,nc]:
                in_open_list[nr,nc] = True
                heapq.heappush(open_list,(g,[nr,nc])) #Se agrega el nodo a la lista abierta con el valor g para ordenar en el heap
            execution_steps += 1
...
def a_star(start_r, start_c, goal_r, goal_c, grid_map, cost_map):
    ...
    [r,c] = [start_r, start_c]
    heapq.heappush(open_list,(0,[start_r, start_c])) #Se agrega el nodo inicial a la lista abierta con un valor f=0
    in_open_list[start_r, start_c] = True
    distances   [start_r, start_c] = 0
    ...
    while len(open_list) > 0 and [r,c] != [goal_r, goal_c]:
        ...
        for [nr,nc] in neighbors:
            if grid_map[nr,nc] > 40 or grid_map[nr,nc] < 0 or in_closed_list[nr,nc]:
                continue
            g = distances[r,c] + 1 + cost_map[nr,nc] #Considerando pesos como en el metodo Dijkstra
            h = abs(nr-goal_r) + abs (nc-goal_c) #Distancia de Manhattan
            f = g + h
            if g < distances[nr,nc]:
                distances[nr,nc]    = g
                parent_nodes[nr,nc] = [r,c]
            if not in_open_list[nr,nc]:
                in_open_list[nr,nc] = True
                heapq.heappush(open_list,(f,[nr,nc])) #Se agrega el nodo a la lista abierta con el valor f para ordenarlo
            execution_steps += 1
...
def get_smooth_path(original_path, alpha, beta):
    ...
    N = len(smooth_path) - 1 #Indice mayor
