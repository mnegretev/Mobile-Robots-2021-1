...
def breadth_first_search(start_r, start_c, goal_r, goal_c, grid_map):
    ...
    [r,c] = [start_r, start_c]#renglon columnas actuales
    open_list.append([start_r, start_c])
    in_open_list[start_r, start_c] = True
    distances   [start_r, start_c] = 0

    while len(open_list) > 0 and [r,c] != [goal_r, goal_c]:#mientras OL no este vacia y nodo actual diferente a nodo meta
        [r,c] = open_list.popleft()  ######## HINT
        in_closed_list[r,c] = True
        neighbors = [[r+1, c],  [r,c+1],  [r-1, c],  [r,c-1]]#renglon arriba-abajo, columna arriba-abajo
        dist = distances[r,c] + 1
        for [nr,nc] in neighbors:
            if grid_map[nr,nc] > 40 or grid_map[nr,nc] < 0 or in_closed_list[nr,nc]:#si es una celda no navegable o ya esta en lista
                continue#nos la brincamos
