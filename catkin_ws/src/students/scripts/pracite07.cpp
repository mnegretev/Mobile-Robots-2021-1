...
geometry_msgs::PoseArray get_initial_distribution(int N, float min_x, float max_x, float min_y, float max_y, float min_a, float max_a)
{
     ...
     * For the Euler angles (roll, pitch, yaw) = (0,0,th) the corresponding quaternion is
     * given by (0,0,sin(th/2), cos(th/2)).
     */
...
void move_particles(geometry_msgs::PoseArray& particles, float delta_x, float delta_y, float delta_t)
{
     ...
     * each particle you need to rotate delta_x and delta_y, on Z axis, an angle th_i, where th_i
...
bool check_displacement(geometry_msgs::Pose2D& robot_pose, geometry_msgs::Pose2D& delta_pose)
{
    ...
    float delta_a = robot_pose.th - last_pose.th;
    if(delta_a >  M_PI) delta_a -= 2*M_PI;
    if(delta_a < -M_PI) delta_a += 2*M_PI;
    if(sqrt(delta_x*delta_x + delta_y*delta_y) > DISTANCE_LIMIT || fabs(delta_a) > ANGLE_THRESHOLD)
    {
        last_pose = robot_pose;
        delta_pose.x =  delta_x*cos(robot_pose.th) + delta_y*sin(robot_pose.th);
        delta_pose.y = -delta_x*sin(robot_pose.th) + delta_y*cos(robot_pose.th);
        delta_pose.th = delta_a;
        return true;
    }
    return false;
}
...
geometry_msgs::Pose2D get_robot_odometry(tf::TransformListener& listener)
{
    ...
    pose.th = atan2(t.getRotation().z(), t.getRotation().w())*2;
    return pose;
}
...
{
    tf::Transform odom_to_base(tf::Quaternion(0,0,sin(odom.th/2),cos(odom.th/2)), tf::Vector3(odom.x,odom.y,0));
    tf::Transform map_to_base(tf::Quaternion(0,0,sin(loc.th/2),cos(loc.th/2)), tf::Vector3(loc.x, loc.y, 0));
...
int main(int argc, char** argv)
{
    ...
    while(ros::ok())
    {
        ...
        if(check_displacement(robot_odom, delta_pose))
        {
            ...
            move_particles(particles, delta_pose.x, delta_pose.y, delta_pose.th);
